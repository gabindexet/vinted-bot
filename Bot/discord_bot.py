...
def build_bot(config: dict) -> commands.Bot:
    ...
    async def watch_add(ctx, *, args: str):
        """!watch add "polo ralph lauren" --max-price 15 --category homme --rotation 7 --margin 120 --channel nike"""
        positional, flags = parse_flags(args)
        if not positional:
            await ctx.send("Il faut au moins un nom/mot-clé entre guillemets. "
                            "Exemple : `!watch add \"polo ralph lauren\" --max-price 15 --channel nike`")
            return
        ...
        alert_channel = flags.get("channel")  # ex: "nike", "carhartt", "vetements"

        if alert_channel:
            existing = get_channel(ctx.guild, alert_channel)
            if not existing:
                await ctx.send(f"⚠️ Salon `#{alert_channel}` introuvable sur ce serveur — "
                                f"vérifie l'orthographe exacte. Watch non créée.")
                return
        ...
        dest = f"#{alert_channel}" if alert_channel else "#alertes-achat (par défaut)"
        await ctx.send(f"✅ Watch **#{watch_id} — {name}** créée "
                        f"(max {max_price or '∅'}€, rotation {rotation}j, marge min {margin}%, "
                        f"alertes → {dest}).{note}")
    ...
    async def watch_list(ctx):
        ...
        for w in watches:
            ...
            chan = f"#{w['alert_channel']}" if w["alert_channel"] else "#alertes-achat"
            lines.append(f"{status} `#{w['id']}` **{w['name']}** — "
                          f"max {w['max_price'] or '∅'}€ · marché {mp} · rotation {w['target_rotation_days']}j · {chan}")
...
async def send_opportunity_embed(bot: commands.Bot, default_channel_name: str, watch, item, price, url, result):
    guild = bot.guilds[0] if bot.guilds else None
    if not guild:
        return
    channel_name = watch["alert_channel"] or default_channel_name
